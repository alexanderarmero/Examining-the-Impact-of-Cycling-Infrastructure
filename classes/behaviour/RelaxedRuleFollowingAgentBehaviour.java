...
public class RelaxedRuleFollowingAgentBehaviour extends TickerBehaviour implements CommunicationModels {
...
// Constructor
public RelaxedRuleFollowingAgentBehaviour(SumoTraciConnection conn, String agentType, double silent, Agent agent, long interval) {
            ...
            switch (agentType) {
                case "vehicle":
                    if (vType.equals("veh__passenger")) {
                           conn.do_job_set(Vehicle.setParameter(vehicleId, "lcCooperative", "0.8"));
                           conn.do_job_set(Vehicle.setParameter(vehicleId, "lcSpeedGain", "0.8"));
                           conn.do_job_set(Vehicle.setParameter(vehicleId, "lcKeepRight", "0.5"));
                        System.out.println("Connected to car: " + vehicleId);

                        // Define values for RelaxedRuleFollowing cars
                        ...
                        // Define values for RelaxedRuleFollowing motorcycles
                        speed = 13.9; // meters per second (50 km/h)
                        acceleration = 1.5; // meters per second squared
                        ...
                        System.out.println("Connected to bus: " + vehicleId);
                           ...
                           conn.do_job_set(Vehicle.setParameter(vehicleId, "lcSpeedGain", "0.8"));
                           conn.do_job_set(Vehicle.setParameter(vehicleId, "lcKeepRight", "0.5"));
                                                // Define values for RelaxedRuleFollowing buses
                ...
                case "cyclist":
                    if (vType.equals("cyc__bicycle")) {
                        ...
                        // Define values for RelaxedRuleFollowing cyclists
...
private void sendMessage(String vehicleId, String agentType) {    
    ...
    if (messageProbability < 0.5) {
        ...
        System.out.println("[" + agentType + " " + vehicleId + "] Sending social message to nearby agents");
        // Implement social message sending logic here
    }
}
